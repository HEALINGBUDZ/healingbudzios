...
public class MediaBrowser2: UIViewController, UIScrollViewDelegate, UIActionSheetDelegate {
    ...
    // Data
    var header_hight_for_IphoneX : CGFloat = 0.0
    private var mediaCount = -1
    private var mediaArray = [Media?]()
    private var thumbMedias = [Media?]()
    /// Provided via init
	private var fixedMediasArray: [Media]?
    var  userShortInfoView:UserProfilShortInfo!
    var bottomBargalleryView:GalleryBottomBarView!
	// Views
    ...
    var savingPost = Media()
    var fromSavingPost = false
    ...
    public weak var delegate: MediaBrowser2Delegate?
    ...
    public var autoPlayOnAppear = true
     ...
     */
    public convenience init(delegate: MediaBrowser2Delegate) {
        self.init()
...
//        navigationController?.view.backgroundColor = UIColor.white
    ...
    public override func viewDidLoad() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.applyFilter), name:NSNotification.Name(rawValue: "MediaReport"), object: nil)
...
//        }
        ...
        var pagingScrollViewFrame = frameForPagingScrollView
        ...
        if displayMediaNavigationArrows {
            ...
            let previousButtonImage = UIImage.imageForResourcePath(
                ...
                inBundle: Bundle(for: MediaBrowser2.self))
            ...
            previousButton = UIBarButtonItem(
                ...
                target: self,
                action: #selector(MediaBrowser2.gotoPreviousPage))
            
            nextButton = UIBarButtonItem(
                image: nextButtonImage,
                style: UIBarButtonItemStyle.plain,
                target: self,
                action: #selector(MediaBrowser2.gotoNextPage))
        }
    ...
    func applyFilter(notification: NSNotification){
        if self.post.flaged_count?.intValue==0
        {
            self.post.flaged_count=NSNumber(integerLiteral: 1)
            self.feedDataController.performlReportOrFlag(index: feedDataController.postList.index(where: {$0.id! == self.post.id!})!, reason: (notification.userInfo?["value"] as? String)!)
        }
    }
...
//                    newBackButton.setTitleTextAttributes([String : AnyObject](), for: .highlighted)
        ...
        if enableGrid {
            ...
            items.append(UIBarButtonItem(
                image: UIImage.imageForResourcePath(name: "UIBarButtonItemGrid", inBundle: Bundle(for: MediaBrowser2.self)),
                style: .plain,
                target: self,
                action: #selector(MediaBrowser2.showGridAnimated)))
        } else {
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        if !viewIsActive && navigationController?.viewControllers[0] as? MediaBrowser2 !== self {
...
//        {
            ...
            if let customView = Bundle.main.loadNibNamed("GalleryBottomBarView", owner: self, options: nil)?.first as? GalleryBottomBarView {
                customView.frame = CGRect(x: 0, y: self.view.frame.size.height-80, width: self.view.frame.size.width, height: 80)
                if userIfno == true {
                ...
                if let count = self.post.likes_count?.intValue{
                    customView.likesButton.isSelected = self.post.isLiked
                    customView.likesCountButton.setTitle(" \(count) Likes", for: .normal)
                }else{
                     customView.likesCountButton.setTitle(" \(0) Likes", for: .normal)
                }
                
                
              
                if  let commentsCount = post.comments_count?.intValue {
                     customView.commentsCountButton.setTitle(" \(commentsCount) Comments", for: .normal)
                }else{
                     customView.commentsCountButton.setTitle(" \(0) Comments", for: .normal)
                }
               
                
                if let  repostCount = self.post.shared_count?.intValue{
                      customView.repostsButton.setTitle(" \(0) Repost\(repostCount > 1 ? "s" : "")", for: .normal)
                }else{
                      customView.repostsButton.setTitle(" \(0) Repost\(0 > 1 ? "s" : "")", for: .normal)
                }
              
                bottomBargalleryView = customView
                customView.repostsButtonAction = {[weak self]  in
                    if self!.post!.user_id!.stringValue != DataManager.sharedInstance.getPermanentlySavedUser()!.ID {
                        customView.increaseRepost()
                        self?.feedDataController.performRepost(cell: self?.cell, index: (self?.index)!, controller: self!, completion:{(r) in
                            if r
                            {
                ...
                customView.likeCount = {[weak self]  in
                    // self?.handleRepostsAction(index: indexPath.row)
                        ...
                        likedActivity.postIndex = self?.index
                        self?.present(likedActivity, animated: true, completion: nil)
                    }
                
                customViews.append(customView)
            }
            if let userProfilShortInfo = Bundle.main.loadNibNamed("UserProfilShortInfo", owner: self, options: nil)?.first as? UserProfilShortInfo {
                userProfilShortInfo.frame = CGRect(x: 0, y:0, width: self.view.frame.size.width, height: 120 + header_hight_for_IphoneX)
                self.view.addSubview(userProfilShortInfo)
                userProfilShortInfo.CloseMenu.isHidden=true
                if userIfno == false {
                    userProfilShortInfo.pointsLabel.isHidden=true
                    userProfilShortInfo.pointsBudzSeperator.isHidden=true
                    userProfilShortInfo.budLabel.isHidden=true
                    userProfilShortInfo.userNameLabel.isHidden=true
                    userProfilShortInfo.budIconImageView.isHidden=true
                    userProfilShortInfo.profilePictureImageView.isHidden=true
                    userProfilShortInfo.isViewPresent = true
                    userProfilShortInfo.btnShare.isHidden = true
                    userProfilShortInfo.dots_btn_width_contraints.constant = 0.0
                     userProfilShortInfo.layoutIfNeeded()
                    userProfilShortInfo.menu_dots.isHidden = true
                    userProfilShortInfo.backgroundColor = UIColor.clear
                }
                userProfilShortInfo.displayUserProfile(post:self.post)
                userProfilShortInfo.menuButtonAction = { [unowned self] menuButton in
                    self.feedDataController.showPopoverMenu(isShownOnTop: false, sender: menuButton, for: self.post, controller: self)    { menuAction in
                        self.handleMoreMenu(for: menuAction)
                    }
                }
                
                userProfilShortInfo.downloadBtn = { [unowned self] downloadbtn in
                   self.downloadImage(sender: downloadbtn)
                }
            userShortInfoView=userProfilShortInfo
                customViews.append(userProfilShortInfo)
            }
...
//        }
    ...
    fileprivate func handleMoreMenu(for action: MenuAction) {
        ...
        case .edit:
            ...
            postFeedViewController.editIndex = self.index
   ...
   fileprivate func muteUnmute()
    {
        ...
        self.post.mute_post_by_user_count = NSNumber(integerLiteral: isMute)
    }
    /**
     view Did Appear
    ...
    func restorePreviousNavBarAppearance(animated: Bool) {
        if let navi = navigationController, didSavePreviousStateOfNavBar {
           ...
           // navBar.titleTextAttributes = [NSAttributedStringKey.foregroundColor:previousNavigationBarTextColor ?? UIColor.black]
            do
            {
                    navBar.titleTextAttributes = try [NSForegroundColorAttributeName:previousNavigationBarTextColor ?? UIColor.black]
            }
    ...
    func layoutVisiblePages() {
        ...
        // ToDo4 Media Boroser Image Size 
        // Get paging scroll view frame to determine if anything needs changing
        var pagingScrollViewFrame = frameForPagingScrollView
        pagingScrollViewFrame.origin.y = 100
        pagingScrollViewFrame.size.height =  pagingScrollViewFrame.size.height - 170
        ...
        for index in iFirstIndex...iLastIndex {
            if !isDisplayingPageForIndex(index: index) {
                ...
                if page.displayingVideo() {
                    ...
                    playButton.setImage(UIImage(named: "PlayButtonOverlayLarge", in: Bundle(for: MediaBrowser2.self), compatibleWith: nil), for: .normal)
                    playButton.setImage(UIImage(named: "PlayButtonOverlayLargeTap", in: Bundle(for: MediaBrowser2.self), compatibleWith: nil), for: .highlighted)
                    ...
                    if(userShortInfoView != nil){
                        userShortInfoView.btnDownload.isHidden = true
                    }
                }else {
                    
                    if(userShortInfoView != nil){
                        imageee = page.photoImageView.image
                        userShortInfoView.btnDownload.isHidden = false
                        userShortInfoView.btnDownload.addTarget(self, action: #selector(downloadImage), for: .touchUpInside)
                    }
                }
                ...
                if self.displaySelectionButtons {
                    ...
                    if let selectedOffImage = mediaSelectedOffIcon {
                        ...
                        selectedButton.setImage(UIImage(named: "ImageSelectedSmallOff", in: Bundle(for: MediaBrowser2.self), compatibleWith: nil), for: .normal)
                    }
                    
                    if let selectedOnImage = mediaSelectedOnIcon {
                        selectedButton.setImage(selectedOnImage, for: .selected)
                    } else {
                        selectedButton.setImage(UIImage(named: "ImageSelectedSmallOn", in: Bundle(for: MediaBrowser2.self), compatibleWith: nil), for: .selected)
                    }
    ...
    @objc func downloadImage (sender: Any) {
        if let photo = mediaAtIndex(index: currentPageIndex) {
        ...
        if imageee != nil {
            self.confirmationCustomeAlert(title: "Are you sure?", discription: "You want to save this Image in your device gallery.", btnTitle1: "Cancel", btnTitle2: "Save") { (isConfirm, btn_nmbr) in
                if isConfirm {
                    UIImageWriteToSavedPhotosAlbum(self.imageee!, self, #selector(self.image(_:didFinishSavingWithError:contextInfo:)), nil)
                }else{
...
//        page.backgroundColor = areControlsHidden ? UIColor.black : UIColor.white
    ...
    func playVideoWhileSaving() {
        ...
        // Get video and play
            savingPost.getVideoURL() { url in
                if let u = url {
                    DispatchQueue.main.async() {
                        self.playVideo(videoURL: u, atPhotoIndex: 1)
                    }
                } else {
                    self.setVideoLoadingIndicatorVisible(visible: false, atPageIndex: 1)
                }
            }
    }
    ...
    func showGrid(animated: Bool) {
        ...
        if let gc = gridController {
            ...
            gc.view.frame = CGRect(x: 0, y: 40, width: bounds.width, height: bounds.height)
            gc.view.alpha = 0.0
    ...
    func hideControlsAfterDelay() {
        if !areControlsHidden {
            ...
            controlVisibilityTimer = Timer.scheduledTimer(
                ...
                target: self,
                selector: #selector(MediaBrowser2.hideControls),
                userInfo: nil,
                repeats: false)
        }
    }
